 ...
 */
var EFFECT_IMAGE = '/img/effect0.gif';
var ICON_IMAGE = '/img/icon0.gif';
var FONT_IMAGE = '/img/font.png';
...
// 合計ラベル
enchant.util.TotalLabel = enchant.Class.create(enchant.util.MutableText, {
	initialize: function(x, y) {
		MutableText.call(this, 0, 0);
		switch(arguments.length){
			case 2:
		...
		this._current = 0;
		this.easing = 2.5;
		this.text = this.label = 'TOTAL:';
		this.addEventListener('enterframe', function(){
			if(this.easing == 0){
				this.text = this.label + (this._current = this._total);
			}else{
				this._current += Math.ceil((this._total - this._current)/this.easing);
				this.text = this.label + this._current;
			}
		});
	},
...
// レベルラベル
enchant.util.LevelLabel = enchant.Class.create(enchant.util.MutableText, {
	initialize: function(x, y) {
		MutableText.call(this, 0, 0);
		switch(arguments.length){
			case 2:
		...
		this._current = 0;
		this.easing = 2.5;
		this.text = this.label = 'LEVEL:';
		this.addEventListener('enterframe', function(){
			if(this.easing == 0){
				this.text = this.label + (this._current = this._level);
			}else{
				this._current += Math.ceil((this._level - this._current)/this.easing);
				this.text = this.label + this._current;
			}
		});
	},
